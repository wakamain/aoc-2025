...
int main() {
  ...
  int parent[2048];
  int size[2048];
  int checks = strcmp(filename, "small-input.txt") == 0 ? 10 : 1000;
  if (checks > edge_cnt) {
    checks = edge_cnt;
  }
  ...
  if (LOG_VERBOSE) {
    printf("INFO: 3 LARGEST SIZES [");
    for (int i = 0; i < 3; i++) {
      if (i < 2) printf("%d, ", sizes[i]);
      else printf("%d", sizes[i]);
    }
    printf("]\n");
  }

  int limit = sz_idx < 3 ? sz_idx : 3;
