...
long long dfs(char *current_key, char *target_key) {
  if (strcmp(current_key, target_key) == 0) return 1;
  if (strcmp(current_key, "out") == 0) return 0;
  
  struct Node *node, *tmp;
  node = NULL;
  HASH_FIND_STR(graph, current_key, node);
  ...
  for (size_t i = 0; i < node->child_count; i++) {
    total_paths += dfs(node->children[i], target_key);
  }
...
int main() {
  ...
  reset_cache();
  long long scenario_one_a = dfs("svr", "dac");
  reset_cache();
  long long scenario_one_b = dfs("dac", "fft");
  reset_cache();
  long long scenario_one_c = dfs("fft", "out");

  long long path_one = scenario_one_a * scenario_one_b * scenario_one_c;

  reset_cache();
  long long scenario_two_a = dfs("svr", "fft");
  reset_cache();
  long long scenario_two_b = dfs("fft", "dac");
  reset_cache();
  long long scenario_two_c = dfs("dac", "out");

  long long path_two = scenario_two_a * scenario_two_b * scenario_two_c;
  
  long long result = path_one + path_two;
